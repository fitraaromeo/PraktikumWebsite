...
// OnMounted: Ambil data detail portfolio berdasarkan ID
onMounted(async () => {
  try {
    const response = await api.get(`/api/portfolios/${route.params.id}`);
    const portfolio = response.data.data;

    // Isi data ke state
    title.value = portfolio.title;
    link.value = portfolio.link;
...
// Fungsi untuk memperbarui data portfolio
const updatePortfolio = async () => {
  ...
  try {
    // Panggil API untuk update portfolio
    await api.post(`/api/portfolios/${route.params.id}`, formData);
